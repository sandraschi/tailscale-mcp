      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400">
            Tailscale tailnet devices (via MCP tool{" "}
            <code className="text-slate-500">manage_tailnet_devices</code>)
          </p>
        </div>
        <Button
          variant="outline"
        ...
        <Card className="border-amber-800 bg-amber-950/30">
          <CardContent className="py-3 text-amber-200 text-sm space-y-2">
            <p>
              ...
              <code className="text-amber-100">TAILSCALE_API_KEY</code> or{" "}
              <code className="text-amber-100">TAILSCALE_TAILNET</code>. Put
              them in the repo root <code className="text-amber-100">.env</code>{" "}
              and restart the uvicorn process (e.g.{" "}
              <code className="text-amber-100">web_sota\start.ps1</code>).
            </p>
        ...
        <Card className="border-amber-800 bg-amber-950/30">
          <CardContent className="py-3 text-amber-200 text-sm space-y-2">
            ...
            <p className="text-slate-400 text-xs">
              Backend: <code className="text-slate-300">{API_BASE}</code> —
              ensure uvicorn is running and reachable from the browser (CORS
              allows 10820 and 5173).
            </p>
          </CardContent>
        </Card>
      )}
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {devices.length === 0 && !loading && !error && (
          <Card className="col-span-full border-slate-800 bg-slate-950/50">
            <CardContent className="py-8 text-center text-slate-400">
              No devices returned. If the backend is up, check API keys and
              tailnet in{" "}
        ...
        {devices.map((device) => {
          const addrList = device.addresses?.length
            ...
            : (device.ip_addresses ?? []);
          const key =
            device.id ?? device.device_id ?? device.name ?? Math.random();
          return (
