...
export function Taildrive() {
  ...
  async function fetchShares() {
    ...
    setError(null);
    try {
      const res = await callTool("manage_taildrop", {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400">
            Share directories across your tailnet
          </p>
        </div>
        <Button
          variant="outline"
        ...
        <CardContent>
          {loading ? (
            ...
            <p className="text-slate-400 text-sm">
              ...
              <code className="block mt-2 text-xs bg-slate-900 rounded p-2">
        ...
        <CardContent className="text-sm text-slate-300 space-y-4">
          <div>
            ...
            <p className="text-slate-400 mb-2">
              Add node attributes to your tailnet policy file:
            </p>
          ...
          <div>
            ...
            <pre className="text-xs bg-slate-900 rounded p-3">
              tailscale drive share repos D:\Dev\repos
            </pre>
          ...
          <div>
            ...
            <p className="text-slate-400 mb-2">
              Shares are available via WebDAV at{" "}
            ...
            <p className="text-slate-400 mb-2">
              Path format:{" "}
              <code className="text-amber-400">/tailnet/machine/share</code>
            </p>
            <p className="text-slate-400">Example mount (Windows):</p>
            <pre className="text-xs bg-slate-900 rounded p-3 mt-1">
              net use Z:
              http://100.100.100.100:8080/tailfab45.ts.net/goliath/repos
              /persistent:yes
            </pre>
          </div>
          ...
          <div>
            <h4 className="text-slate-200 font-medium mb-1">
              4. Control access with ACL grants
            </h4>
            <pre className="text-xs bg-slate-900 rounded p-3 overflow-x-auto">
              {JSON.stringify(
                {
          ...
          <div>
            ...
            <div className="space-y-1 text-xs font-mono">
              <div>
                <span className="text-green-400">share</span> &lt;name&gt;
                &lt;path&gt; — Create/modify a share
              </div>
              <div>
                <span className="text-green-400">list</span> — List current
                shares
              </div>
              <div>
                <span className="text-green-400">rename</span> &lt;old&gt;
                &lt;new&gt; — Rename a share
              </div>
              <div>
                <span className="text-green-400">unshare</span> &lt;name&gt; —
                Remove a share
              </div>
            </div>
          </div>
