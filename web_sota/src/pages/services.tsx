...
export function Services() {
  ...
  async function fetchServices() {
    ...
    setError(null);
    try {
      const res = await callTool("manage_tailnet_network", {
        operation: "services_list",
      });
      const data = res.data as
        | { services?: ServiceRow[]; count?: number }
        | undefined;
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400">
            Tailscale Services (TailVIPs) – app-aware connectivity
          </p>
        </div>
        <Button
          variant="outline"
      ...
      {error && (
        <Card className="border-amber-800 bg-amber-950/30">
          <CardContent className="py-3 text-amber-200 text-sm">
            {error}. Ensure backend is running and tailnet has Services
            (TailVIPs) enabled.
          </CardContent>
        </Card>
      )}
      ...
      {services.length === 0 && !loading && !error && (
        <Card className="border-slate-800 bg-slate-950/50">
          <CardContent className="py-8 text-center text-slate-400">
            No services. Use manage_tailnet_network with services_create, or
        ...
        {services.map((svc) => (
          <Card key={svc.id} className="border-slate-800 bg-slate-950/50">
            ...
            <CardContent className="space-y-3">
              ...
              {((svc.tailvip_ipv4 ?? svc.tailvipIPv4) ||
                (svc.tailvip_ipv6 ?? svc.tailvipIPv6)) && (
                <div className="text-xs text-slate-400 space-y-1">
                  {(svc.tailvip_ipv4 ?? svc.tailvipIPv4) && (
                    <div>IPv4: {svc.tailvip_ipv4 ?? svc.tailvipIPv4}</div>
                  )}
                  {(svc.tailvip_ipv6 ?? svc.tailvipIPv6) && (
                    <div>IPv6: {svc.tailvip_ipv6 ?? svc.tailvipIPv6}</div>
                  )}
                </div>
              )}
              ...
              {svc.endpoints && svc.endpoints.length > 0 && (
                <div className="text-xs text-slate-500">
                  ...
                  {svc.endpoints
                    .map((ep) => `${ep.port ?? "?"}/${ep.protocol ?? "tcp"}`)
                    .join(", ")}
                </div>
              )}
            </CardContent>
