    ...
    async def tailscale_status(
        ...
        try:
            ...
            if component == "overview" or component is None:
                ...
                response["summary"] = (
                    f"Your Tailscale network has {devices_total} device{'s' if devices_total != 1 else ''} "
                    f"({devices_online} online, {devices_total - devices_online} offline)"
                )

                if alerts_count > 0:
                    response["summary"] += (
                        f" with {alerts_count} alert{'s' if alerts_count != 1 else ''} that need attention"
                    )
                ...
                response["summary"] = (
                    f"Device status check complete: {device_count} device{'s' if device_count != 1 else ''} found"
                )
                if device_count == 0:
                    response["suggestion"] = (
