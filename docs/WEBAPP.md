...
| `/partner-tailnets` | **People & sharing**: members vs tailnet-shared users (`summarize_partner_tailnets` `summary`), Mermaid overview, JSON tabs |
...
1. **Mermaid**  
   - Prefers diagram text from **`get_tailnet_status`** when called with **`include_mermaid_diagram: true`** (field `mermaid_diagram` in the tool result).  
   - If absent, builds a simple **flowchart** from **`manage_tailnet_devices`** `operation: "list"`.  
